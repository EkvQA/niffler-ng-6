...
public class RegistrationWebTest {
    ...
    @Test
    @DisplayName("Регистрация нового пользователя: Проверка успешной регистрации и отображение кнопки входа")
    void shouldRegisterNewUser() {
        ...
        new RegistrationPage()
                ...
                .submit()
                .checkSuccess(successText)
                .checkSignInButton();
    }

    @Test
    @DisplayName("Регистрация существующего пользователя: Проверка ошибки о существующем имени")
    void shouldNotRegisterNewUserWithExistingUsername() {
        Selenide.open(CFG.frontUrl(), LoginPage.class)
                .goToCreateAccountPage();

        new RegistrationPage()
                .fillRegistrationForm(existingUsername, password, password)
                .submit()
                .checkTextError(errorTextUser);
    }

    @Test
    @DisplayName("Ошибка регистрации: Проверка ошибки при несовпадении паролей")
    void shouldShowErrorIfPasswordAndConfirmPasswordAreNotMatch() {
        Selenide.open(CFG.frontUrl(), LoginPage.class)
                .goToCreateAccountPage();

        new RegistrationPage()
                .fillRegistrationForm(secondUsername, password, anotherPassword)
                .submit()
                .checkTextError(errorTextPassword);
    }
}
